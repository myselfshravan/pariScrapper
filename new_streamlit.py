...
# Initialize session state to store full history if not already present
...
try:
    response = requests.get("http://192.168.68.104:5000/odds")
    if response.status_code == 200:
        new_record = response.json()
        # Append new record if it's new based on the timestamp
...
if st.session_state["data"]:
    ...
    # Compute margin for each record and add as a new column
